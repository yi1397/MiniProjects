...
int main() try {
	...
	commands.push_back(std::make_pair(std::make_pair(1, 2), "πÆ¿Ã ø≠∏≥¥œ¥Ÿ."));
	commands.push_back(std::make_pair(std::make_pair(6, 3), "√¢πÆ¿Ã ø≠∏≥¥œ¥Ÿ."));
	...
	while (1)
	{
		...
		/*
		std::cout << "∏Ì∑…æÓ ∞πºˆ" << commandTime.size() << std::endl;
		std::cout << "∏Ì∑…æÓ: " << command << std::endl;
		
		for (int i = 0; i < commandTime.size(); i++)
		{
			std::cout << ((command & (1 << i)) ? "∞Ì∞≥»ÁµÎ " : "¥´ ∞®∫˝¿”");
		}
		std::cout<<std::endl;
		*/
